...
const DonutChart = ({ percent }) => {
  ...
  return (
    <svg width="90" height="90" viewBox="0 0 90 90" style={{ flexShrink: 0 }}>
      ...
      <circle cx="45" cy="45" r={r} stroke="url(#donutGrad)" strokeWidth="10" fill="none"
...
const ProgressBar = ({ percent, color }) => {
  ...
  return (
    <div className={styles.pbTrack}>
      <div className={styles.pbFill} style={{ width: `${width}%`, background: color || undefined }} />
    </div>
  );
};
      ...
      <main className={styles.main}>
        ...
        <div className={styles.gridTwo}>
          <div className={styles.progressCard}>
            <div className={styles.cardTitle}><span>📊</span> Progres Belajar</div>
          ...
          <div className={styles.streakCard}>
            <div className={styles.streakTop}>
              ...
              <div className={styles.streakDots}>
                {weekDays.map((d, i) => (
                  <div key={i} className={`${styles.streakDot} ${i < streakDays ? styles.done : ''}`} title={d}>
                    {i < streakDays ? '✓' : d}
                  </div>
                ))}
              </div>
            </div>
            <div className={styles.quoteBox}>"{todayQuote}"</div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className={styles.moduleGrid}>
            {modules.map((mod, i) => {
              ...
              return (
                <div key={mod.id} className={styles.moduleCard} onClick={() => navigate('/learning')}>
                  <div className={styles.moduleIconWrap} style={{ background: mod.iconBg }}>{mod.emoji}</div>
        ...
        <div className={styles.badgeSection}>
          ...
          <div className={styles.badgeGrid}>
            {allBadges.map((b, i) => (
              <div key={i} className={`${styles.badgeChip} ${b.earned ? styles.earned : styles.locked}`}>
